...
class Handler:  # Abstract handler
    ...
    def handle(self, request):
        handled = self._handle(request)  # If handled, stop hare
    ...
    def _handle(self, request):
        raise NotImplemented('Must provide implementation in subclass!')
...
class Client:  # Using handlers
    def __init__(self):
        self.handler = ConcreteHandler1(DefaultHandler(None))  # Create handlers and use then in a sequence you want
